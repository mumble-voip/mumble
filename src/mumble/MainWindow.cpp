...
void MainWindow::serverDisconnected(QAbstractSocket::SocketError err, QString reason) {
	...
	if (!Global::get().sh->qlErrors.isEmpty()) {
		...
		bool matched = true;
		// The default value of the save password dialog option.
		bool save = !Global::get().s.bSuppressIdentity;
		switch (rtLast) {
			...
			case MumbleProto::Reject_RejectType_WrongUserPW:
				pw = PasswordDialog::getPassword(this, tr("Wrong certificate or password"),
