...
#    Requirements:
#        python 2.6
#        PIL 1.1.7 (only if avatar import is enabled)
#        MySQLdb
...
#
#--- Parse parameters and load configuration 
#    Not the best style to do this here but we need the path to Murmur.ice to import it before
#    deriving our classes
#
...
parser.add_option('--nodetach', action='store_true',dest = 'detach',
                  help = 'do not detach from terminal', default = False)
...
class phpBBauthenticator(Murmur.ServerUpdatingAuthenticator):
    ...
    def idToTexture(self, id, current = None):
        ...
        try:
            handle = urllib2.urlopen(url, timeout = 2.0)
            file = StringIO.StringIO(handle.read())
            handle.close()
        except urllib2.URLError, e:
            warning('Image download for "%s" (%d) failed: %s', url, id, e.reason)
            return FALL_THROUGH
...
if __name__ == '__main__':    
    ...
    try:
        ...
        context = daemon.DaemonContext(detach_process = option.detach)
        context.__enter__()
        try:
            do_main_program()
        finally:
