...
static void about(HWND h) {
	::MessageBox(h, L"Reads audio position information from COD5 (v 1.0.1017)", L"Mumble COD5 Plugin", MB_OK);
}
...
static int fetch(float *pos, float *front, float *top) {
	...
	/*
		This plugin uses the following Variables:
			...
			===================================
			0x008CD034		float	Z-Coordinate
			0x008CD038		float	X-Coordinate
			0x008CD03C		float	Y-Coordinate
			0x008CD044		float	Horizontal view (degrees)
			0x008CD040		float	Vertical view (degrees)

			0x0095EAAC		byte	Magical state value
	*/
	ok = peekProc((BYTE *) 0x0095EAAC, &state, 1); // Magical state value
	if (! ok)
		return false;
	/*
		state value is:
	...
	*/
	...
	ok = peekProc((BYTE *) 0x008CD034, pos+2, 4) &&	//Z
	     peekProc((BYTE *) 0x008CD038, pos, 4) &&	//X
	     peekProc((BYTE *) 0x008CD03C, pos+1, 4) && //Y
	     peekProc((BYTE *) 0x008CD044, &viewHor, 4) && //Hor
	     peekProc((BYTE *) 0x008CD040, &viewVer, 4); //Ver
