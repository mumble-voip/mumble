...
#	endif
	...
	if (Global::get().s.bUpdateCheck) {
		// MUMBLE-TFAR: VersionCheck queries this fork's GitHub releases —
		// the upstream check would advertise vanilla Mumble builds without
		// TFAR. Quiet unless a new version is found.
		new VersionCheck(false, Global::get().mw);
	}
