...
std::shared_ptr< CryptStateFactory > UDPVoiceProtocol::m_cryptStateFactory;
...
UDPVoiceProtocol::UDPVoiceProtocol(const std::string &s) : VoiceProtocol(s) {
	...
	if (!m_cryptStateFactory) {
		return;
	}

	std::string cipher_name = s.substr(strlen("MUMBLE_UDP_"));
	CipherType cipher       = m_cryptStateFactory->getCipherType(cipher_name);
