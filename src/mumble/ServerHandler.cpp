...
void ServerHandler::requestUserStats(unsigned int uiSession, bool statsOnly, const QStringList &temporaryAccessTokens) {
	...
	for (const QString &token : temporaryAccessTokens) {
		mpus.add_temporary_access_tokens(token.toUtf8().constData());
	}
...
void ServerHandler::startListeningToChannel(unsigned int channel, const QStringList &temporaryAccessTokens) {
	startListeningToChannels({ channel }, temporaryAccessTokens);
}
...
void ServerHandler::stopListeningToChannel(unsigned int channel, const QStringList &temporaryAccessTokens) {
	stopListeningToChannels({ channel }, temporaryAccessTokens);
}
...
void ServerHandler::requestBanList(const QStringList &temporaryAccessTokens) {
	MumbleProto::BanList mpbl;
	mpbl.set_query(true);

	for (const QString &token : temporaryAccessTokens) {
		mpbl.add_temporary_access_tokens(token.toUtf8().constData());
	}

	sendMessage(mpbl);
}

void ServerHandler::requestUserList(const QStringList &temporaryAccessTokens) {
	MumbleProto::UserList mpul;

	for (const QString &token : temporaryAccessTokens) {
		mpul.add_temporary_access_tokens(token.toUtf8().constData());
	}

	sendMessage(mpul);
}

void ServerHandler::requestACL(unsigned int channel, const QStringList &temporaryAccessTokens) {
	MumbleProto::ACL mpacl;
	mpacl.set_channel_id(channel);
	mpacl.set_query(true);

	for (const QString &token : temporaryAccessTokens) {
		mpacl.add_temporary_access_tokens(token.toUtf8().constData());
	}

	sendMessage(mpacl);
}

void ServerHandler::registerUser(unsigned int uiSession, const QStringList &temporaryAccessTokens) {
	MumbleProto::UserState mpus;
	mpus.set_session(uiSession);
	mpus.set_user_id(0);
...
void ServerHandler::kickBanUser(unsigned int uiSession, const QString &reason, bool ban,
	...
	for (const QString &token : temporaryAccessTokens) {
		mpur.add_temporary_access_tokens(token.toUtf8().constData());
	}

	sendMessage(mpur);
}

void ServerHandler::sendUserTextMessage(unsigned int uiSession, const QString &message,
										const QStringList &temporaryAccessTokens) {
	MumbleProto::TextMessage mptm;
	mptm.add_session(uiSession);
	mptm.set_message(u8(message));

	for (const QString &token : temporaryAccessTokens) {
		mptm.add_temporary_access_tokens(token.toUtf8().constData());
	}

	sendMessage(mptm);
}

void ServerHandler::sendChannelTextMessage(unsigned int channel, const QString &message, bool tree,
										   const QStringList &temporaryAccessTokens) {
	MumbleProto::TextMessage mptm;
	if (tree) {
		...
		if (message == QString::fromUtf8(Global::get().ccHappyEaster + 10))
...
void ServerHandler::removeChannel(unsigned int channel, const QStringList &temporaryAccessTokens) {
	MumbleProto::ChannelRemove mpcr;
	mpcr.set_channel_id(channel);

	for (const QString &token : temporaryAccessTokens) {
		mpcr.add_temporary_access_tokens(token.toUtf8().constData());
	}

	sendMessage(mpcr);
}

void ServerHandler::addChannelLink(unsigned int channel, unsigned int link, const QStringList &temporaryAccessTokens) {
	MumbleProto::ChannelState mpcs;
