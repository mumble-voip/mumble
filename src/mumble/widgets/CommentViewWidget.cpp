...
void CommentViewWidget::updateCommentView(ClientUser *user, Channel *channel) {
	if (user) {
		...
		if (!user->qbaTextureHash.isEmpty() && user->qbaTexture.isEmpty()) {
			user->qbaTexture = Global::get().textureManager->convertTexture(Global::get().db->blob(user->qbaTextureHash), user->qbaTextureFormat);
		...
		connect(user, &ClientUser::localNicknameChanged, this, [=]() { updateCommentContent(user, nullptr); });
		connect(user, &ClientUser::commentChanged, this, [=]() { updateCommentContent(user, nullptr); });
		...
		connect(channel, &Channel::channelEntered, updateCommentSubscriptionRaw,
				[=](const Channel *newChannel, [[maybe_unused]] const Channel *prevChannel,
		...
		connect(channel, &Channel::channelExited, updateCommentSubscriptionRaw,
				[=](const Channel *c, [[maybe_unused]] const User *u) { updateCommentContent(nullptr, c); });
	}
}
