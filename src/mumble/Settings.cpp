...
void Settings::load(const QString &path, bool noBackupPrompt) {
	...
	try {
		...
		if (!mumbleQuitNormally && !noBackupPrompt) {
			...
			if (!path.endsWith(QLatin1String(BACKUP_FILE_EXTENSION))) {
				...
				if (backupInfo.exists() && backupInfo.isFile()) {
					...
					if (msgBox.exec() == QMessageBox::Yes) {
						...
						load(backupPath, noBackupPrompt);
					}
				}
			} else {
		...
		if (!path.endsWith(QLatin1String(BACKUP_FILE_EXTENSION)) && QFileInfo(path + BACKUP_FILE_EXTENSION).exists()) {
			...
			load(path + BACKUP_FILE_EXTENSION, noBackupPrompt);
		}
	}
...
void Settings::load(bool noBackupPrompt) {
	...
	if (foundExisting) {
		...
		load(settingsPath, noBackupPrompt);
		...
		load(settingsPath + BACKUP_FILE_EXTENSION, noBackupPrompt);
	} else {
