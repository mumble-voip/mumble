...
static bool selfSignedServerCert_SHA1_RSA_2048(QSslCertificate &qscCert, QSslKey &qskKey) {
	...
	unsigned char *commonName = reinterpret_cast<unsigned char *>(const_cast<char *>("Murmur Autogenerated Certificate v2"));
	...
	if (add_ext(x509, NID_basic_constraints, "critical,CA:FALSE") == 0) {
		ok = false;
		goto out;
	}

	if (add_ext(x509, NID_ext_key_usage, "serverAuth,clientAuth") == 0) {
		ok = false;
		goto out;
	}

	if (add_ext(x509, NID_subject_key_identifier, "hash") == 0) {
		ok = false;
		goto out;
	}

	if (add_ext(x509, NID_netscape_comment, "Generated from murmur") == 0) {
		ok = false;
		goto out;
	}
