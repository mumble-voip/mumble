...
void LookConfig::setActiveThemes(const std::optional< ThemeInfo::StyleInfo > configuredStyle,
	...
	if (configuredStyle) {
		for (int i = 0; i < qcbLightTheme->count(); ++i) {
			QVariant themeData = qcbLightTheme->itemData(i);
			if (themeData.canConvert< ThemeInfo::StyleInfo >()) {
				...
				if (style == configuredStyle.value()) {
					qcbLightTheme->setCurrentIndex(i);
					break;
				}
			}
		}
	} else {
		qcbLightTheme->setCurrentIndex(0); // "None"
	}

	if (configuredDarkStyle) {
		for (int i = 0; i < qcbDarkTheme->count(); ++i) {
			QVariant themeData = qcbDarkTheme->itemData(i);
			if (themeData.canConvert< ThemeInfo::StyleInfo >()) {
				ThemeInfo::StyleInfo style = themeData.value< ThemeInfo::StyleInfo >();
				if (style == configuredDarkStyle.value()) {
					qcbDarkTheme->setCurrentIndex(i);
					break;
				}
			}
		}
	} else {
		qcbDarkTheme->setCurrentIndex(0); // "None"
	}
}
