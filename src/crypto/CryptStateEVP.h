...
public:
	explicit CryptStateEVP(const EVP_CIPHER *c);
	~CryptStateEVP() override;
...
private:
	...
	static constexpr unsigned int TAG_LENGTH = 0;

	const EVP_CIPHER *cipher;

	unsigned char *raw_key;
	unsigned char *encrypt_iv;
	unsigned char *decrypt_iv;
	unsigned char *decrypt_history;
};


template< typename T > CryptStateEVP< T >::CryptStateEVP(const EVP_CIPHER *c) : CryptState(), cipher(c) {
	raw_key         = new unsigned char[T::KEY_LENGTH];
	encrypt_iv      = new unsigned char[T::IV_LENGTH];
	decrypt_iv      = new unsigned char[T::IV_LENGTH];
...
template< typename T > CryptStateEVP< T >::~CryptStateEVP() {
	delete raw_key;
	delete encrypt_iv;
	delete decrypt_iv;
	delete decrypt_history;
}
