...
static int fetch(float *avatar_pos, float *avatar_front, float *avatar_top, float *camera_pos, float *camera_front, float *camera_top, std::string &context, std::wstring &identity) {
	...
	// Avatar pointers
	procptr32_t avatar_base = peekProc<procptr32_t>(pModule32 + 0x016FEC04);
	...
	ok = peekProc(avatar_offset + 0x0, &avatar_pos_corrector, 12) && // Avatar Position values (Z, Y and X).
			peekProc(pModule32 + 0x016FEE40, &camera_pos_corrector, 12) && // Camera Position values (Z, Y and X).
			peekProc(avatar_offset + 0xC, &avatar_front_corrector, 12) && // Avatar Front values (Z, Y and X).
			peekProc(pModule32 + 0x016FEE28, &camera_front_corrector, 12) && // Camera Front Vector values (Z, Y and X).
			peekProc(pModule32 + 0x016FEE34, &camera_top_corrector, 12); // Camera Top Vector values (Z, Y and X).
