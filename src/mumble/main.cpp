...
#	endif
	...
	std::string custom_jack_client_name;
	bool print_license{ false };
	bool print_authors{ false };
	bool print_third_party_licenses{ false };
	bool dump_input_streams{ false };
	bool print_echocancel_queue{ false };
  ...
  std::string locale;
	...
	CLI::Option *option_default_certificate_dir =
		app.add_option("--default-certificate-dir", default_certificate_dir,
					   ...
					   "from the settings.\n");
	CLI::Option *option_translation_dir = app.add_option("--translation-dir", extra_translation_dir,
	...
	app.add_flag("-m, --multiple", bAllowMultiple, "Allow multiple instances of the client to be started.\n");
	app.add_flag("-n, --noidentity", suppressIdentity, "Suppress loading of identity files (i.e., certificates.)\n");
	app.add_flag("--license", print_license, "Show the Mumble license.\n");
	app.add_flag("--authors", print_authors, "Show the Mumble authors.\n");
	app.add_flag("--third-party-licenses", print_third_party_licenses,
				 "Show licenses for third-party software used by Mumble.\n");
	app.add_flag("--dump-input-streams", dump_input_streams,
				 "Dump PCM streams at various parts of the input chain\n"
				 "(useful for debugging purposes)\n"
				 "- raw microphone input\n"
				 "- speaker readback for echo cancelling\n"
				 "- processed microphone input\n");
	app.add_flag("--print-echocancel-queue", print_echocancel_queue,
				 "Print on stdout the echo cancellation queue state\n"
				 "(useful for debugging purposes)\n");
	app.add_flag("--print-translation-dirs", printTranslationDirs,
				 "Print out the paths in which Mumble will search for\n"
				 "translation files that overwrite the bundled ones.\n"
				 "(Useful for translators testing their translations)\n");
	app.add_flag("--version", print_version, "Print version information and exit");

	CLI::App *rpc_subcommand = app.add_subcommand("rpc", "It is possible to remote control a running instance\n"
														 "of Mumble by using the 'mumble rpc' command.\n"
														 "\n"
														 "Valid actions are:\n"
														 "  mute\n"
														 "                Mute self\n"
  ...
  if (*rpc_subcommand) {
	...
	if (*option_window_title_ext) {
		if (window_title_postfix.empty()) {
	...
	if (*option_translation_dir) {
		if (extra_translation_dir.empty()) {
