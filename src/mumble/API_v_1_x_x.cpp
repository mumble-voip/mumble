...
void MumbleAPI::isLocalUserUsingPTT_v_1_3_x(mumble_plugin_id_t callerID, bool *isUsingPTT,
											std::shared_ptr< api_promise_t > promise) {
	if (QThread::currentThread() != thread()) {
		// Invoke in main thread
		QMetaObject::invokeMethod(this, "isLocalUserUsingPTT_v_1_3_x", Qt::QueuedConnection,
								  Q_ARG(mumble_plugin_id_t, callerID), Q_ARG(bool *, isUsingPTT),
								  Q_ARG(std::shared_ptr< api_promise_t >, promise));
...
void MumbleAPI::getLocalUserTalkingState_v_1_3_x(mumble_plugin_id_t callerID, mumble_talking_state_t *talkingState,
												 std::shared_ptr< api_promise_t > promise) {
	if (QThread::currentThread() != thread()) {
		// Invoke in main thread
		QMetaObject::invokeMethod(this, "getLocalUserTalkingState_v_1_3_x", Qt::QueuedConnection,
								  Q_ARG(mumble_plugin_id_t, callerID), Q_ARG(mumble_talking_state_t *, talkingState),
								  Q_ARG(std::shared_ptr< api_promise_t >, promise));
...
void MumbleAPI::requestStartLocalUserWhisperShout_v_1_3_x(mumble_plugin_id_t callerID, mumble_connection_t connection,
	...
	if (QThread::currentThread() != thread()) {
		// Invoke in main thread
		QMetaObject::invokeMethod(this, "requestStartLocalUserWhisperShout_v_1_3_x", Qt::QueuedConnection,
								  Q_ARG(mumble_plugin_id_t, callerID), Q_ARG(mumble_connection_t, connection),
								  Q_ARG(const mumble_userid_t *, users), Q_ARG(const size_t, userCount),
								  Q_ARG(const mumble_channelid_t *, channels), Q_ARG(const size_t, channelCount),
								  Q_ARG(uint32_t *, allocID), Q_ARG(std::shared_ptr< api_promise_t >, promise));
	...
	if (!mw) {
		...
		*allocID = mw->allocatePluginTarget();
		...
		for (size_t c = 0; c < channelCount; c++) {
			...
			if (Channel::get(static_cast< unsigned int >(channel))) {
...
void MumbleAPI::requestStopLocalUserWhisperShout_v_1_3_x(mumble_plugin_id_t callerID, mumble_connection_t connection,
														 const uint32_t allocID,
														 std::shared_ptr< api_promise_t > promise) {
	if (QThread::currentThread() != thread()) {
		// Invoke in main thread
		QMetaObject::invokeMethod(this, "requestStopLocalUserWhisperShout_v_1_3_x", Qt::QueuedConnection,
								  Q_ARG(mumble_plugin_id_t, callerID), Q_ARG(mumble_connection_t, connection),
								  Q_ARG(const uint32_t, allocID), Q_ARG(std::shared_ptr< api_promise_t >, promise));
...
#define TYPED_ARGS                                                                                                     \
	mumble_plugin_id_t callerID, mumble_connection_t connection, const mumble_userid_t *users, const size_t userCount, \
