...
sub format_body {
  my $self = shift;
  my ($msg, $files, $author, $qunklist) = @_;
  ...
  if ( $No_Wrap and ! $Summary ) {
    ...
    if ( $FSF_Style ) {
      ...
      my $author_add = ($author eq "slicer") ? "" : ("[".$author."]");
      $msg =~ s/\n$//;
      $msg = $self->preprocess_msg_text($msg . $author_add);
      $body = $files . $After_Header . $msg;
    ...
    foreach my $time (@key_list)
    {
      ...
      FOOBIE:
        ...
        for my $msg (sort keys %$mesghash)
        {
          ...
          my $body = $self->format_body($msg, $files, $author, \@qunklist);
